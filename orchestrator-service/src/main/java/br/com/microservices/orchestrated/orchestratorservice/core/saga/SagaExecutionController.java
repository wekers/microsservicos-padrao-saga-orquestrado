...
public class SagaExecutionController {
    ...
    public ETopics getNextTopic(Event event){
        ...
        var topic = findTopicBySouceAndStatus(event);
        logCurrentSaga(event,topic);
        return topic;
    }

    private ETopics findTopicBySouceAndStatus(Event event){
    ...
    private void logCurrentSaga(Event event, ETopics topic){
        ...
        switch (event.getStatus()){
            case SUCCESS -> log.info("### CURRENT SAGA: {} | SUCCESS | NEXT TOPIC {} | {}",
                    source, topic, sagaId);
            case ROLLBACK_PENDING -> log.info("### CURRENT SAGA: {} | SENDING TO ROLLBACK SERVICE | NEXT TOPIC {} | {}",
                    source, topic, sagaId);
